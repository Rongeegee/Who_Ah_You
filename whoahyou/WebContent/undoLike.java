...
@WebServlet("/undoLike")
public class undoLike extends HttpServlet{
	...
	protected boolean deleteLike(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException, SQLException{
		...
		String deleteSQL = "DELETE FROM likes WHERE Liker = ? AND Likee = ? AND Date_Time = ?";
		PreparedStatement deleteSt = DBcon.conn.prepareStatement(deleteSQL);  
		deleteSt.setString(1, likerr);
		deleteSt.setString(2, likeee);
		deleteSt.setString(3, time);
